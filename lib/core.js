...
(function (root, factory) {
  ...
  const parseNumericCell = (value) => {
    ...
    // comma, no dot, and not a thousands group like "1,520".
    if (!str.includes('.') && (str.match(/,/g) || []).length === 1) {
      const [, decimals] = str.split(',');
      if (decimals.length !== 3) str = str.replace(',', '.');
    }
  ...
  // Coerces a possibly-empty setting at the point of use.
  ...
  const resolveColumns = (headers) => {
    ...
    // Anything still unmatched falls back to this field's conventional slot,
    // but only if that column was not already claimed by a named match.
    POSITIONAL_FIELDS.forEach((field, slot) => {
      if (mapping[field] !== undefined) return;
      if (slot < headers.length && !taken.has(slot)) {
  ...
  const buildRow = (record, mapping, index) => {
    ...
    const pValue = parseNumericCell(cell('pValue'));
  ...
  const buildRowsFromRecords = (records, headers) => {
    ...
    return { rows, mapping, matchedByName, unreadableCount };
  };
  ...
  const validateProject = (project) => {
    ...
    const validatePlot = (plot, index) => {
      if (!plot || typeof plot !== 'object') {
        throw new Error(`Plot ${index + 1} is not valid.`);
      }
      if (plot.id === undefined || plot.id === null) {
        throw new Error(`Plot ${index + 1} is missing its id.`);
      }
      if (!Array.isArray(plot.data)) {
        throw new Error(`Plot ${index + 1} is missing its data rows.`);
      }
      if (!plot.settings || typeof plot.settings !== 'object') {
        throw new Error(`Plot ${index + 1} is missing its settings.`);
      }
    };
    ...
    if (Array.isArray(project.plots)) {
      ...
      project.plots.forEach(validatePlot);
      return { kind: 'current', plots: project.plots, globalSettings: project.globalSettings || null };
    }

    if (Array.isArray(project.data)) {
      return { kind: 'legacy', data: project.data, settings: project.settings || {} };
    }
