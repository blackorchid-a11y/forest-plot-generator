...
(function (root, factory) {
  ...
  // ln Gamma(z) by the Lanczos approximation (g = 7), accurate to ~1e-15.
  const LANCZOS = [
  ...
  const logGamma = (z) => {
    ...
    const zm = z - 1;
    let sum = LANCZOS[0];
    for (let i = 1; i < LANCZOS.length; i++) sum += LANCZOS[i] / (zm + i);
    const t = zm + 7.5;
    return 0.5 * Math.log(2 * Math.PI) + (zm + 0.5) * Math.log(t) - t + Math.log(sum);
  };

  // Regularised upper incomplete gamma Q(a, x): a series below a + 1 and a
  // continued fraction above it (Numerical Recipes, gammq).
  const gammaUpperRegularized = (a, x) => {
    if (x <= 0) return 1;
  ...
  const chiSquareUpperTail = (x, df) => {
    ...
    return gammaUpperRegularized(df / 2, x / 2);
  };

  const POOLING_MODELS = ['fixed', 'random'];

  // Inverse-variance meta-analysis on the log scale, fixed-effect or
  // DerSimonian-Laird random-effects, with Cochran's Q and I^2.
  ...
  const computePooledEffect = (rows, model = 'fixed') => {
    const studies = [];
    rows.forEach((d) => {
      ...
      studies.push({ logOR: Math.log(or), variance: se * se });
    });

    if (studies.length === 0) return null;
    const pooledWith = (tau2) => {
      const weights = studies.map((st) => 1 / (st.variance + tau2));
      const total = weights.reduce((sum, w) => sum + w, 0);
      const estimate = studies.reduce((sum, st, i) => sum + weights[i] * st.logOR, 0) / total;
      return { weights, total, estimate, se: Math.sqrt(1 / total) };
    };

    const fixed = pooledWith(0);
    if (!Number.isFinite(fixed.total) || fixed.total <= 0) return null;

    // Heterogeneity is measured against the fixed-effect estimate.
    const df = studies.length - 1;
    const q = studies.reduce((sum, st, i) => sum + fixed.weights[i] * (st.logOR - fixed.estimate) ** 2, 0);
    const sumSquaredWeights = fixed.weights.reduce((sum, w) => sum + w * w, 0);
    const c = fixed.total - sumSquaredWeights / fixed.total;
    const tau2 = df > 0 && c > 0 ? Math.max(0, (q - df) / c) : 0;
    const i2 = df > 0 && q > 0 ? Math.max(0, (q - df) / q) : 0;

    const useRandom = model === 'random';
    const chosen = useRandom ? pooledWith(tau2) : fixed;
    if (!Number.isFinite(chosen.estimate) || !Number.isFinite(chosen.se)) return null;

    return {
      model: useRandom ? 'random' : 'fixed',
      or: Math.exp(chosen.estimate),
      lowerCI: Math.exp(chosen.estimate - Z_95 * chosen.se),
      upperCI: Math.exp(chosen.estimate + Z_95 * chosen.se),
      pValue: 2 * normalUpperTail(chosen.estimate / chosen.se),
      ...
      heterogeneity: {
        ...
        i2,
        tau2,
        pValue: df > 0 ? chiSquareUpperTail(q, df) : null
      }
    };
  };

  // "Heterogeneity: tau^2 = 0.369; I^2 = 88%; p = 0.004", or '' for a single
  // study, where heterogeneity is undefined. tau^2 is only meaningful to the
  // reader of a random-effects model, so it is shown only there.
  const describeHeterogeneity = (pooled) => {
    if (!pooled || !pooled.heterogeneity || pooled.heterogeneity.df < 1) return '';
    const { i2, tau2, pValue } = pooled.heterogeneity;
    const parts = [];
    if (pooled.model === 'random') parts.push(`τ² = ${formatEstimate(tau2)}`);
    parts.push(`I² = ${Math.round(i2 * 100)}%`);
    const p = formatPValue(pValue);
    if (p) parts.push(p.startsWith('<') ? `p < ${p.slice(1)}` : `p = ${p}`);
    return `Heterogeneity: ${parts.join('; ')}`;
  };

  const poolingModelLabel = (model) => (model === 'random' ? 'Random-effects model' : 'Fixed-effect model');
  ...
  const generateSmartTicks = (minVal, maxVal, scale) => {
    ...
    if (scale === 'log') {
      ...
      // Past three decades the 2x and 5x ticks crowd into each other
      // ("0.0010.002"), so a wide axis gets powers of ten only.
      const intermediate = [];
      const decades = Math.log10(maxVal / minVal);
      for (let i = 0; decades <= 3 && i < majors.length - 1; i++) {
        [2, 5].forEach((factor) => {
